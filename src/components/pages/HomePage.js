...
const HomePage = () => (
  <main>
    <div className="page-container jumbotron jumbotron-fluid text-dark bg-light animated fadeIn">
      ...
      <p>This test should be timeboxed for around one to two hours. Dont worry if you don't complete everything.

Zip up your project and send it back - Or push to a github repo and make it accessable for someone to clone and build so we can review.</p>
      <br />
      <br />
      <h3>Tasks:</h3>
      <ul>
        <li>Complete the `FetchZipCodesReducer.js` so that the data is stored on success.</li>
        <li>Call the `fetchZipCodesAction` somewhere in the code so that the service is called.</li>
        <li>Create a component / set of components which will display a list of zipcodes on the page when the data is successfully returned and stored in redux state.</li>
      </ul>
      <h3>Bonus tasks:</h3>
      <ul>
        <li>Style the app to make it responsive at desktop and mobile device screen sizes.</li>
        <li>Import your preferred test runner ( e.g. jest, mocha) and write some unit tests for your new components.</li>
      </ul>
    </div>
  </main >
);
